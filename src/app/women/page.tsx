  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-[#F4F4F1] text-[#635F58]">
      ...
      <main className="flex-1 pt-24 sm:pt-28 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        {/* Editorial Header */}
        <div className="text-center py-6 sm:py-8 border-b border-[#635F58]/20">
          ...
          <p className="text-xs sm:text-sm font-light tracking-wide text-[#635F58]/70 mt-2 max-w-lg mx-auto">
            Understated elegance, fluid draping, and considered forms designed to move effortlessly with the body.
          </p>
        </div>

        {/* Filter and Control Bar */}
        <div className="py-6 flex flex-col md:flex-row justify-between items-center gap-4 border-b border-[#635F58]/20 mb-8">
              ...
              <button
                ...
                className={`text-[11px] uppercase tracking-widest px-3 py-1 border transition-all cursor-pointer ${
                  selectedCategory.toLowerCase() === cat.value.toLowerCase()
                    ? 'bg-[#635F58] text-[#F4F4F1] border-[#635F58]'
          ...
          <div className="flex items-center gap-4 text-[11px]">
            <span className="text-[#635F58]/60 uppercase tracking-widest">
              {products.length} {products.length === 1 ? 'Product' : 'Products'}
            </span>

            <div className="flex items-center gap-1.5">
              <label htmlFor="women-sort" className="uppercase tracking-wider text-[#635F58]/60 hidden sm:inline">
                Sort:
              </label>
              <select
                id="women-sort"
                ...
                className="bg-transparent border border-[#635F58]/20 px-2.5 py-1 text-xs uppercase tracking-wider text-[#635F58] focus:outline-none cursor-pointer"
              >
        ...
        {loading ? (
          <div className="py-24 text-center">
            <div className="inline-block w-6 h-6 border-2 border-[#635F58] border-t-transparent rounded-full animate-spin mb-3"></div>
            <p className="text-xs uppercase tracking-widest text-[#635F58]/60">
              Loading collection...
            </p>
          </div>
        ) : currentProducts.length === 0 ? (
          <div className="py-24 text-center">
            <p className="text-xs uppercase tracking-widest text-[#635F58]/60 mb-4">
              No products found in this category.
            </p>
            <button
              onClick={() => setSelectedCategory('All')}
              className="text-xs uppercase tracking-wider underline text-[#635F58] hover:opacity-75 cursor-pointer"
            >
              View All Women
            </button>
          </div>
                ...
                <Link href={`/product/${product.slug}`} className="block">
                  <div className="relative aspect-[3/4] mb-3 bg-[#e6e2d8]/40 overflow-hidden">
                    ...
                    {product.inStock === false && (
                      <span className="absolute top-2 left-2 bg-[#635F58] text-[#F4F4F1] text-[10px] uppercase tracking-widest px-2 py-0.5">
                        Sold Out
                      </span>
                    )}
                  </div>
                  <div className="flex justify-between items-start">
                    <h2 className="text-xs sm:text-sm font-normal text-[#635F58] group-hover:opacity-60 transition-opacity">
                      {product.name}
                    </h2>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-[#F4F4F1]">
        <div className="inline-block w-6 h-6 border-2 border-[#635F58] border-t-transparent rounded-full animate-spin"></div>
      </div>
    }>
      <WomenContent />
    </Suspense>
