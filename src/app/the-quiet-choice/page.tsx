    ...
    <div className="min-h-screen flex flex-col justify-between bg-[#F4F4F1] text-[#635F58]">
      ...
      <main className="flex-1 pt-28 sm:pt-36 pb-24 px-5 sm:px-8 lg:px-12 max-w-6xl mx-auto w-full flex flex-col justify-start">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 sm:mb-20">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-light tracking-[0.18em] uppercase text-[#635F58] font-serif mb-4">
            The Quiet Choice
          </h1>
          <p className="text-xs sm:text-sm tracking-wider text-[#635F58]/75 font-light leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10 lg:gap-14">
          {cards.map((card) => (
            <Link
              key={card.title}
              href={card.href}
              className="group flex flex-col justify-between bg-[#F4F4F1] border border-[#635F58]/20 p-6 sm:p-8 lg:p-10 transition-all duration-300 hover:border-[#635F58]/40 hover:shadow-lg"
            >
              <div>
                {/* Card Visual Image */}
                <div className="relative w-full aspect-[4/3] sm:aspect-[16/10] overflow-hidden bg-[#F4F4F1]/40 mb-6 sm:mb-8">
                  <Image
                    src={card.image}
                    alt={card.title}
                    fill
                  ...
                  />
                  <div className="absolute inset-0 bg-[#635F58]/5 group-hover:bg-transparent transition-colors duration-300"></div>
                </div>
                ...
                {/* Title */}
                <h2 className="text-xl sm:text-2xl font-light font-serif tracking-wider uppercase text-[#635F58] mb-3 group-hover:opacity-80 transition-opacity">
                  {card.title}
                </h2>

                {/* Description */}
                <p className="text-xs sm:text-[13px] text-[#635F58]/75 font-light leading-relaxed mb-8">
                  {card.description}
                </p>
              </div>

              {/* Action Link / Arrow */}
              <div className="pt-4 border-t border-[#635F58]/20 flex items-center justify-between text-xs uppercase tracking-[0.2em] font-medium text-[#635F58]">
